    ...
    <el-card class="mt-3" shadow="never">
      <template #header>
        <div class="card-header">
          ...
          <el-button :loading="loading" @click="fetchPermissions" type="primary" size="small">
            {{ $t('actions.refresh') }}
          </el-button>
        </div>
      </template>
      ...
      <div v-else>
        <el-empty v-if="modules.length === 0" :description="$t('system.noPermissions')" />
        <el-collapse v-else v-model="activeModules">
          <el-collapse-item v-for="module in modules" :key="module" :name="module">
            <template #title>
...
const fetchPermissions = async () => {
  ...
  try {
    ...
    if (res.data?.success) {
      ...
      modules.value = res.data.data.modules || Object.keys(groupedPermissions.value);
