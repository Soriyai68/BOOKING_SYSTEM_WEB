  ...
  <div class="user-list">
    ...
    <el-card>
      <div class="toolbar">
        ...
        />
        <el-select v-model="statusFilter" :placeholder="$t('users.filterByStatus')" clearable>
      ...
      >
        ...
        <el-table-column prop="role" :label="$t('users.role')" width="100">
          <template #default="{ row }">
            <el-tag :type="row.role === 'admin' || row.role === 'superadmin' ? 'warning' : 'info'">
              {{ $t(`users.${row.role}`) }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="created_at" :label="$t('users.created')" width="120">
          <template #default="{ row }">
            {{ formatDate(row.created_at) }}
          </template>
        </el-table-column>
        <el-table-column :label="$t('users.actions')" width="120">
          <template #default="{ row }">
