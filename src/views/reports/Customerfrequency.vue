  ...
  <div class="customer-frequency">
    <el-card>
      ...
      >
        ...
        <el-table-column prop="customer_name" :label="$t('reports.customerName')" min-width="150">
          <template #default="{ row }">
            <span>{{ row.customer_name || 'N/A' }}</span>
          </template>
        </el-table-column>

        <el-table-column prop="customer_phone" :label="$t('reports.phone')" min-width="130">
          <template #default="{ row }">
            <span>{{ row.customer_phone || 'N/A' }}</span>
          </template>
        </el-table-column>

        <el-table-column prop="customer_email" :label="$t('reports.email')" min-width="180">
          <template #default="{ row }">
        ...
        >
          <template #default="{ row }">
            <el-tag type="primary" size="large">{{ row.total_bookings }}</el-tag>
          </template>
        </el-table-column>

        <el-table-column 
          prop="total_spend" 
          :label="$t('reports.totalSpend')" 
          width="150" 
          sortable
          align="right"
        >
...
const loadData = async () => {
  loading.value = true;
  try {
    const response = await reportService.getCustomerBookingFrequency();
...
onMounted(() => {
  appStore.setBreadcrumbs([
    ...
    { title: t('reports.title'), path: '/admin/reports' },
    { title: t('reports.customerFrequency'), path: '/admin/reports/customer-frequency' }
  ]);
